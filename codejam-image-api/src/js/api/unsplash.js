...
async function getLinkToImage(town = 'Nizhny Novgorod', size = 128) {
  const resizeQuery = `fit=fill&fill=solid&fill-color=c4c4c4&w=${size}&h=${size}`;
  try {
    const response = await fetch(
      `${API_URL}${GET_RND_PHOTO}?${resizeQuery}&query=town,${town}&client_id=${API_ACCESS_KEY}`,
    );
    if (!response.ok) {
